...
// Public Constants
CGFloat const PLVImageViewController_DefaultAlphaForBackgroundDimmingOverlay = 0.66f;
CGFloat const PLVImageViewController_DefaultBackgroundBlurRadius = 2.0f;

// Private Constants
static CGFloat const PLVImageViewController_MinimumBackgroundScaling = 0.94f;
static CGFloat const PLVImageViewController_TargetZoomForDoubleTap = 3.0f;
static CGFloat const PLVImageViewController_MaxScalingForExpandingOffscreenStyleTransition = 1.25f;
static CGFloat const PLVImageViewController_TransitionAnimationDuration = 0.3f;
static CGFloat const PLVImageViewController_MinimumFlickDismissalVelocity = 800.0f;
...
} PLVImageViewControllerStartingInfo;
...
@interface PLVImageViewController ()
<
...
// General Info
@property (strong, nonatomic, readwrite) PLVImageInfo *imageInfo;
@property (strong, nonatomic, readwrite) UIImage *image;
@property (assign, nonatomic, readwrite) PLVImageViewControllerTransition transition;
@property (assign, nonatomic, readwrite) PLVImageViewControllerMode mode;
@property (assign, nonatomic, readwrite) PLVImageViewControllerBackgroundOptions backgroundOptions;
@property (assign, nonatomic) PLVImageViewControllerStartingInfo startingInfo;
@property (assign, nonatomic) PLVImageViewControllerFlags flags;
...
@implementation PLVImageViewController
...
- (instancetype)initWithImageInfo:(PLVImageInfo *)imageInfo
    ...
    if (self) {
        ...
        if (_mode == PLVImageViewControllerMode_Image) {
...
- (void)showFromViewController:(UIViewController *)viewController
    ...
    if (self.mode == PLVImageViewControllerMode_Image) {
        if (transition == PLVImageViewControllerTransition_FromOffscreen) {
    ...
    } else if (self.mode == PLVImageViewControllerMode_AltText) {
...
- (void)dismiss:(BOOL)animated {
    ...
    if (self.mode == PLVImageViewControllerMode_AltText) {
        [self dismissByExpandingAltTextToOffscreenPosition];
    }
    else if (self.mode == PLVImageViewControllerMode_Image) {
        
        if (_flags.imageIsFlickingAwayForDismissal) {
            [self dismissByCleaningUpAfterImageWasFlickedOffscreen];
        }
        else if (self.transition == PLVImageViewControllerTransition_FromOffscreen) {
            ...
            BOOL useCollapsingThumbnailStyle = (startingRectForThumbnailIsNonZero
                                                && self.image != nil
                                                && self.transition != PLVImageViewControllerTransition_FromOffscreen);
...
- (NSUInteger)supportedInterfaceOrientations {
    ...
    /*
     ...
     PLVImageViewController while the **device** is oriented in
     ...
     the rotation corrections during presentation. - August 31, 2014 PLV.
     */
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        ...
        self.lastUsedOrientation = [UIApplication sharedApplication].statusBarOrientation;
        PLVImageViewControllerFlags flags = self.flags;
        flags.isRotating = NO;
        self.flags = flags;
    }];
}
...
- (void)deviceOrientationDidChange:(NSNotification *)notification {
    ...
    /*
     ...
     This makes it difficult to preserve the desired behavior of PLVImageViewController. 
...
- (void)setupImageAndDownloadIfNecessary:(PLVImageInfo *)imageInfo {
    ...
    if (imageInfo.image) {
        ...
        NSURLSessionDataTask *task = [PLVSimpleImageDownloader downloadImageForURL:imageInfo.imageURL canonicalURL:imageInfo.canonicalImageURL completion:^(UIImage *image) {
...
- (void)showImageViewerByExpandingFromOriginalPositionFromViewController:(UIViewController *)viewController {
    ...
    if (self.backgroundOptions & PLVImageViewControllerBackgroundOption_Blurred) {
    ...
    [viewController presentViewController:self animated:NO completion:^{
        ...
        CGFloat duration = PLVImageViewController_TransitionAnimationDuration;
        if (USE_DEBUG_SLOW_ANIMATIONS == 1) {
            duration *= 4;
        }
        
        PLVImageViewController *weakSelf = self;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [UIView
                 ...
                 animations:^{
                     ...
                     if ([UIApplication sharedApplication].PLV_usesViewControllerBasedStatusBarAppearance) {
                     ...
                     CGFloat scaling;
                     if (!(weakSelf.backgroundOptions & PLVImageViewControllerBackgroundOption_Scaled)) {
                         scaling = 1.0;
                     } else {
                         scaling = PLVImageViewController_MinimumBackgroundScaling;
                     }
                     ...
                     if (weakSelf.backgroundOptions & PLVImageViewControllerBackgroundOption_Blurred) {
...
- (void)showImageViewerByScalingDownFromOffscreenPositionWithViewController:(UIViewController *)viewController {
    ...
    [viewController presentViewController:self animated:NO completion:^{
        ...
        [self updateScrollViewAndImageViewForCurrentMetrics];
        CGFloat scaling = PLVImageViewController_MaxScalingForExpandingOffscreenStyleTransition;
            ...
            [UIView
             ...
             animations:^{
                 ...
                 if (!(weakSelf.backgroundOptions & PLVImageViewControllerBackgroundOption_Scaled)) {
                     ...
                     targetScaling = PLVImageViewController_MinimumBackgroundScaling;
                 }
...
- (CGFloat)alphaForBackgroundDimmingOverlay {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(alphaForBackgroundDimmingOverlayInImageViewer:)]) {
        ...
        alpha = PLVImageViewController_DefaultAlphaForBackgroundDimmingOverlay;
    }
...
- (CGFloat)backgroundBlurRadius {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(backgroundBlurRadiusForImageViewer:)]) {
        ...
        blurRadius = PLVImageViewController_DefaultBackgroundBlurRadius;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                if ([UIApplication sharedApplication].PLV_usesViewControllerBasedStatusBarAppearance == NO) {
                    [[UIApplication sharedApplication] setStatusBarHidden:_startingInfo.statusBarHiddenPriorToPresentation
...
- (void)dismissByExpandingAltTextToOffscreenPosition {
    ...
    [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        CGFloat targetScale = PLVImageViewController_MaxScalingForExpandingOffscreenStyleTransition;
...
- (UIView *)blurredSnapshotFromParentmostViewController:(UIViewController *)viewController {
    ...
    // the performance of PLV_applyBlurWithRadius:tintColor:saturationDeltaFactor:maskImage:
    ...
    //UIImage *blurredImage = [image PLV_applyBlurWithRadius:blurRadius tintColor:nil saturationDeltaFactor:1.0f maskImage:nil];
...
- (void)updateLayoutsForCurrentOrientation {
    ...
    if (_flags.rotationTransformIsDirty) {
        ...
        if (_flags.isPresented) {
            ...
            CGFloat targetScaling;
            if (!(self.backgroundOptions & PLVImageViewControllerBackgroundOption_Scaled)) {
                targetScaling = 1.0;
            } else {
                targetScaling = PLVImageViewController_MinimumBackgroundScaling;
            }
...
- (void)updateScrollViewAndImageViewForCurrentMetrics {
    ...
    BOOL usingOriginalPositionTransition = (self.transition == PLVImageViewControllerTransition_FromOriginalPosition);
...
- (void)imageDoubleTapped:(UITapGestureRecognizer *)sender {
    ...
    if (self.scrollView.zoomScale == 1.0f) {
        ...
        CGFloat zoomWidth = self.view.bounds.size.width / PLVImageViewController_TargetZoomForDoubleTap;
        CGFloat zoomHeight = self.view.bounds.size.height / PLVImageViewController_TargetZoomForDoubleTap;
        targetZoomRect = CGRectMake(point.x - (zoomWidth/2.0f), point.y - (zoomHeight/2.0f), zoomWidth, zoomHeight);
        targetInsets = [self contentInsetForScrollView:PLVImageViewController_TargetZoomForDoubleTap];
    } else {
        self.scrollView.accessibilityHint = self.accessibilityHintZoomedOut;
    ...
    [CATransaction begin];
    __weak PLVImageViewController *weakSelf = self;
...
- (void)dismissingPanGestureRecognizerPanned:(UIPanGestureRecognizer *)panner {
    ...
    if (panner.state == UIGestureRecognizerStateBegan) {
        ...
        if (vectorDistance > PLVImageViewController_MinimumFlickDismissalVelocity) {
            if (_flags.isDraggingImage) {
