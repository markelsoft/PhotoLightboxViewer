...
ALAssetsFilter * ALAssetsFilterFromQBImagePickerControllerFilterType(QBImagePickerControllerFilterType type) {
    ...
    switch (type) {
        ...
        case QBImagePickerControllerFilterTypePhotos:
            ...
            //NSLog(@"return # of photos....");
            return [ALAssetsFilter allPhotos];
            break;
            
        case QBImagePickerControllerFilterTypeVideos:
            
            //NSLog(@"return # of videos....");
...
- (instancetype)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        self.filterType = QBImagePickerControllerFilterTypeNone;      // no filter shows all...
...
- (void)setOnlyPhotosTitle
{
    self.title = @"Only Photos";
}

- (void)setPhotosAndVideosTitle
{
    self.title = @"Photos & Videos";
}

- (void)setOnlyVideosTitle
{
    self.title = @"Only Videos";
}
...
- (void)loadAssetsGroupsWithTypes:(NSArray *)types completion:(void (^)(NSArray *assetsGroups))completion
{
    ...
    //NSLog(@"type count %d assets groups %d", types.count, assetsGroups.count);
        ...
        [self.assetsLibrary enumerateGroupsWithTypes:[type unsignedIntegerValue]
                                          usingBlock:^(ALAssetsGroup *assetsGroup, BOOL *stop) {
                                              if (assetsGroup) {
                                                  ...
                                                  [assetsGroup setAssetsFilter:ALAssetsFilterFromQBImagePickerControllerFilterType(self.filterType)];
                                                  
                                                  if (assetsGroup.numberOfAssets > 0) {
                                                      // Add assets group
                                                      //NSLog(@"assetGroups %@", assetsGroups);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    assetsCollectionViewController.newMediaAge = self.newMediaAge;
